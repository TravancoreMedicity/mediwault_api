            ...
            if (err) {
                ...
                if (err instanceof multer.MulterError) {
                    ...
                    if (err.code === "LIMIT_FILE_SIZE") {
                        return res.status(200).json({ success: 0, message: "File too large. Max size is 10MB." });
                    }
                ...
                if (err === "Error: Only .png, .jpg, .jpeg and .pdf files are allowed!") {
                    return res.status(200).json({ success: 0, message: err });
                }

                // Unknown error
                return res.status(200).json({ success: 0, message: "An unknown error occurred during file upload." });
            }
